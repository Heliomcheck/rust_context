...
pub async fn create_item_list_handler(
    ...
    let has_perm = has_permission(&state.db_pool, event_id, user_id, 2).await?; // EDIT_EVENT или CREATE_MODULE
    if !has_perm {
...
pub async fn delete_item_list_handler(
    ...
    let has_perm = has_permission(&state.db_pool, event_id, user_id, 4).await?; // DELETE_EVENT
    if !has_perm {
