...
/// Проверка принадлежности опроса событию (активный)
pub async fn verify_poll_in_event(
    ...
    let row = sqlx::query!(
        "SELECT EXISTS(SELECT 1 FROM poll WHERE poll_id = $1 AND event_id = $2 AND is_active = true) as \"exists!\"",
        poll_id, event_id
    )
    .fetch_one(pool)
    .await
